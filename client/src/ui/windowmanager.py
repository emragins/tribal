...
class WindowManager(manager.Manager):
	...
	def Update(self):
		...
		if mouse.LeftClick():
			win = self.EstablishWindow(x,y)
			if win != self.currentWindow:
				#currentWindow can be None
				try:
					self.currentWindow.LostFocus()
				except:
					pass
		...
		if mouse.LeftDoubleClick():
			## MAYBE can change this to just use current window.. 
			...
			if self.currentWindow is None:
				...
				self.currentWindow.ReceivedLeftDoubleClick(x,y)
